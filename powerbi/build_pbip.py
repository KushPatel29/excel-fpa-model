...
A PBIR report is one JSON file per visual, and this one has over a hundred
of them. Typing those by hand is how a report ends up carrying three different
...
PROJECT = "PGEUtilityFPA"
THEME = "UtilityCanvasDark.json"
...
FORMAT_RULES: tuple[tuple[tuple[str, ...], str], ...] = (
    (("_pct",), "0.0%"),
    (("r_squared",), "0.000"),
    # Regression coefficients before the degree-day rule: "per_hdd" is MWh per
    # customer per degree day, and "#,0" would print every one of them as 0.
    (("intercept", "per_hdd", "per_cdd", "trend_per_year", "plan_use"), "0.000000"),
    (("_cents",), "0.00"),
    (("price", "cost", "revenue", "amount", "impact", "income", "value", "per_customer"),
     "\\$#,0.00"),
    (("mwh", "customers", "hdd", "cdd", "days"), "#,0"),
)
...
def format_for(column: str, dtype: str) -> str:
    if dtype == "dateTime":
        return "yyyy-mm-dd"
    if dtype == "int64":
        # A count of MWh or dollars still wants its thousands separator; a year,
        # a month count or a sort key must not have one.
        lowered = column.lower()
        for stems, fmt in FORMAT_RULES:
            if "%" not in fmt and any(stem in lowered for stem in stems):
...
def infer_columns(path: Path) -> list[dict]:
    ...
    for name in frame.columns:
        ...
        if name.endswith("month") or name in ("date", "week_start", "last_price_change"):
            ...
            # A label is text even when every value is digits: "2017" on a
            # category axis, not 2,017 on a continuous one.
...
LABEL_TOKENS = {
    ...
    "a": "A", "b": "B", "hdd": "HDD", "cdd": "CDD", "mwh": "MWh", "eia": "EIA",
    "ferc": "FERC", "pge": "PGE",
}
...
def visual_json(spec: dict, index: int) -> dict:
    ...
    if kind == "matrix" and spec.get("totals") is False:
        # A grand total down a column of full-year outlooks, or across twelve
        # years of a P&L, is a number nobody reported. Off where it means nothing.
    ...
    for name, meta in TABLES.items():
        ...
        if not csv_path.exists():
            raise FileNotFoundError(
                f"{csv_path} is missing. Run `python model/export_tables.py` first."
            )
