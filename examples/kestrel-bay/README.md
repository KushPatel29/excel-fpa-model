# Kestrel Bay Provisions: an FP&A model in Excel (synthetic distributor)

> **The repository's second Excel model.** It came before the utility model and is kept because it
> shows a different side of the job: working capital, stock close to its shelf life, and channel
> economics for a food distributor. Its data is **synthetic** (seeded, with a story planted to be
> found). The build code and its 182 tests are preserved at the tag
> [`kestrel-bay-v1`](https://github.com/KushPatel29/excel-fpa-model/tree/kestrel-bay-v1), where
> `python -m pytest` still runs them against this workbook.
...
**[Download the workbook](Kestrel_Bay_FPA_Model.xlsx)** (Microsoft 365 Excel) ·
**[Board pack PDF](board_pack.pdf)** (11 pages, exported by Excel)

![Dashboard: KPI tiles, written summary, revenue by month, EBITDA bridge, margin variance by category, scenarios](img/dashboard.png)
...
<table>
<tr><td><img src="img/pvm.png" alt="Price-volume-mix by segment"></td><td><img src="img/scenarios.png" alt="Scenario table, price by volume sensitivity grid, tornado"></td></tr>
<tr><td><img src="img/pnl.png" alt="Monthly P&L: outlook, budget, prior year, variance"></td><td><img src="img/working-capital.png" alt="Working capital trend and shelf-life risk"></td></tr>
<tr><td><img src="img/channels.png" alt="Channel economics by formula and by the data model"></td><td><img src="img/checks.png" alt="31 checks, all passing"></td></tr>
</table>
...
- **An independent twin.** [`model/reference.py`](https://github.com/KushPatel29/excel-fpa-model/blob/kestrel-bay-v1/model/reference.py) recomputes every published
  figure in pandas from the same CSVs, without reading the workbook.
  [`tests/test_workbook_ties_out.py`](https://github.com/KushPatel29/excel-fpa-model/blob/kestrel-bay-v1/tests/test_workbook_ties_out.py) reads the saved workbook's
...
Check out the tag first: `git checkout kestrel-bay-v1`. The commands below describe that tree.
...
```bash
python model/build_workbook.py        # Kestrel_Bay_FPA_Model.xlsx + docs/board_pack.pdf
python model/render_previews.py       # img/*.png from the PDF (needs pymupdf, pillow)
```
