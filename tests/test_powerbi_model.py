...
MODEL = PBIP / "PGEUtilityFPA.SemanticModel" / "definition"
REPORT = PBIP / "PGEUtilityFPA.Report" / "definition"
...
def test_the_project_was_generated():
    assert (PBIP / "PGEUtilityFPA.pbip").exists(), (
...
def test_only_the_data_path_varies_between_machines(tmp_path):
    ...
    generated = out_dir / "PGEUtilityFPA.SemanticModel" / "definition" / "expressions.tmdl"
    committed = PBIP / "PGEUtilityFPA.SemanticModel" / "definition" / "expressions.tmdl"
...
def test_currency_and_percent_measures_carry_a_format_string():
    ...
    unformatted = [
        ...
        # Captions are text: the sentence under a KPI tile, not a number.
        if not fmt and folder != "09 Captions"
    ]
...
def test_the_month_dimension_is_not_marked_as_a_date_table():
    ...
    months = pd.read_csv(ROOT / "tables" / "dim_month.csv")["month_index"].tolist()
